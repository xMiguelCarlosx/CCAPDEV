...
function initializeBookingSystem() {
    ...
    document.getElementById('add').addEventListener('click', function () {
        const currentDate = document.querySelector('.date input[type="date"]').value; // get selected date
        const checkedSeats = Array.from(availableSeatsList.querySelectorAll('input:checked'));
        checkedSeats.forEach(seat => {
            if (selectedTimeSlot && currentDate) { // check if both time slot and date are selected
                const slotWithDate = `${currentDate} ${selectedTimeSlot} - ${seat.value}`;
                const seatItem = document.createElement('li');
                seatItem.innerHTML = `
                    <input type="checkbox" value="${slotWithDate}" unchecked>
                    <span>${slotWithDate}</span>`;
...
document.getElementById('anonymousCheckbox').addEventListener('change', function() {
    var isAnonymous = this.checked;

    sessionStorage.setItem('isAnonymous', isAnonymous);

    var userNameInput = document.getElementById('user-name');
    userNameInput.disabled = isAnonymous;
    userNameInput.value = isAnonymous ? 'Anonymous' : '';
});

function updateUserInputField() {
    var isAnonymous = sessionStorage.getItem('isAnonymous') === 'true';
    var userNameInput = document.getElementById('user-name');
    userNameInput.disabled = isAnonymous;
    userNameInput.value = isAnonymous ? 'Anonymous' : '';
}
...
function initializeTechnicianBookingSystem() {
    ...
    if (student) {
        ...
        document.getElementById('add').addEventListener('click', function () {
            const currentDate = document.querySelector('.date input[type="date"]').value;
